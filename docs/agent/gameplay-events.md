...
Protocol v3 introduced the fixed 64-byte event retained by protocol v4: sequence `u64`, kind `u16`,
...
floor rip, firelock consideration, and turf destruction request. Reaction auxiliary values are
plasma, hydrogen, tritium, and freon. The current service only produces diagnostic fixtures; the
gameplay enum is a validated foundation, not evidence that a callback family has migrated.
